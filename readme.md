 ...
 * 1 case of overwrite bounds of a span of memory
 ...
 * xmlNode: lightweight struct used in both the parsing of the xml and the stages thereafter.
 ...
 * HuffmanCoding: Takes a probability distribution and encodes it into a varying length string of bits, similar to morse code.
...
The parsing of the DAE is initially done in serial, as it's not clear how the most efficient way to parse a heirarchical data structure in parallel, and doing so in serial may be more efficient in terms of computing power. However this is extremely lightweight parsing only the start and end index and the children of each node. Then, the real heavy weight processing, such as converting the character arrays to floats, can be done in parallel.
...
The main difficulty with this apparoach is the **significant** nondeterminism of NVIDIA GPUs, preventing simple de-duplication of the triangles responsible for the artifact
