    ...
    static void calculateDerivsAndMeanEntropies(std::vector<float> dat, gmm &entDeriv, int n, int *maxEntropyGaussian,
        ...
        for (int i = 0; i < dat.size(); i++) {
            for (int j = 0; j < toCalcFor->means.size(); j++) {
                ...
                entDeriv.width[j] += softMaxDeriv * (((valTakeMean * valTakeMean) - thisWidth * thisWidth) /
                                                     (thisWidth * thisWidth * thisWidth));
            }
        }
    }
        ...
        for (int t = 0; t < iterations; t++) {
            ...
            if (iterations > 100) {
                if (t % (iterations / 100) == 0) {
                    std::cout << (int) (t / ((float) (iterations / 100.0))) << "%: mean log ent: " << sumLogEnt
                              << " nats"
                                 "\t" << "mean softmax log ent: " << sumSoftMaxLogEnt / ((double) n) << " nats"
                              << std::endl;
                }
            } else {
